...
public enum NotifyController {
	...
	public boolean sendSms(final String to, final String text) {
		try {
			new URL("http://bytehand.com:3800/send?id=" + byteHandId + "&key="
					+ bytehandKey + "&to="
					+ URLEncoder.encode(to, encodeCharset) + "&from="
					+ URLEncoder.encode(bytehandFrom, encodeCharset) + "&text="
					+ URLEncoder.encode(text, encodeCharset)).getContent();
			return true;
		} catch (Exception e) {
			return false;
		}
